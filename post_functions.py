...
def send_api_request(cookies_input, url_input, data_input):

    test = [checkers.is_not_empty(cookies_input.get()),checkers.is_not_empty(url_input.get()), len(data_input.get('1.0', END)) != 1]
    ...
    if test.count(True) == 3:
        ...
        response = requests.post(url=url_input.get().replace("\'", ""), data=json.loads(data_input.get('1.0', END)),
